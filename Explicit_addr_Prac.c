...
//***** 바로 앞 매크로인 NEXT_BLKP, PREV_BLKP과 변수 이름이 혼동될 것 같습니다. 조금 길지만 NEXT_FREEBLKP 또는 PRED, SUCC 정도로 지었어도 좋았을 것 같습니다!
...
/*------------------------------------- INIT ---------------------------------*/
//***** 저부터가 많이 부족해서 리뷰가 도움이 되었을 지 모르겠네요 ㅎ.. 
//***** 전체적으로 코드가 많이 깔끔한 것 같습니다. 멋있습니다! 
//***** 이따 코드 설명 한번 들으러 가겠습니다. 수고 많으셨습니다~ 

// prev와 next가 들어가므로 Prologue Block을 4word 배치한다.
int mm_init(void)
{   
    //***** 이 부분은 저랑 다르게 구현하셔서 혹시 제가 잘못 이해하고 있으면 말씀해주시면 감사하겠습니다!
    //***** 가용 연결 리스트에 처음과 끝 부분에 미리 prologue, epilogue블록을 넣어서 가장자리 조건을 신경 안써도 되게 만드신 것 같은데 아이디어가 좋은 것 같습니다. 

    //***** 총 9W를 request해야하는 것 같은데 5W만 request하는 것 같습니다. 
    //***** 아래 5 * WSIZE를 9 * WSIZE로 바꿔봤는데 seg fault가 뜨네요.. 
    //***** 참고로 홀수 개의 W를 할당받으면 주소가 더블워드로 정렬이 안될 수도 있을 것 같습니다. 프롤로그 블록 앞에 패딩을 없애서 짝수 개로 맞출 수 있을 것 같습니다. 
    ...
    ///***** 위에서 5W만큼 할당을 받았으면 아래 epilogue 영역은 할당을 받지 않은 공간에 값을 넣고 있는 것 같습니다. 
...
static void *extend_heap(size_t words)
{
    ...
    //***** size 만큼의 메모리를 할당 받아서 size만큼 다 채워주면 새로운 epligue 블록을 위한 영역은 없을 것 같은데 음.. 이따 설명 들으러 가겠습니다~
    // 새로운 epilogue 배치
...
//***** 함수들을 굉장히 깔끔하게 잘 짜시는 것 같습니다! 한수 배우고 갑니다.
...
//***** 연결리스트에 앞 뒤에 미리 블록을 넣어두어서 그런지 굉장히 깔끔하네요. 
